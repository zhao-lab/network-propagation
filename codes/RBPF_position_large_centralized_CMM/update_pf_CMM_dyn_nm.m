function update_pf_CMM_dyn_nm(pose_measure,svxyzmat,sigma_thermal2,orgxyz,Np,v_id,hor_DLP,H,velocity)  % The last argument is the vehicle id
global pf;
...
for k=1:Np
    pf(k).mu{v_id}(1) = pf(k).mu{v_id}(1) + velocity(1,v_id);
    pf(k).mu{v_id}(3) = pf(k).mu{v_id}(3) + velocity(2,v_id);
    pose_local=[pf(k).mu{v_id}(1);pf(k).mu{v_id}(3)];
    %pose_xyz=enu2xyz(pose_local,orgxyz);  %give xyz as column vector
    a=size(H,1);  %calculate the acutal number of satellites
    pre_bias=inv(H.'*H)*H.'*pf(k).common(1:a);
    %position_pre(:,k)=pose_local+pre_bias(1:2);
    position_pre(1,k)=pose_local(1,:) + pre_bias(1);  %predicted pr should include the common error
    position_pre(2,k)=pose_local(2,:) + pre_bias(2);
...
for j=1:Np
    ...
    %     for k=1:sv_n
    %         if y(j,k)==1
    Q=pf(j).cov{v_id}([1,3],[1,3])+hor_DLP*sigma_thermal2;
    D2(j)=innovation(:,j)'*inv(Q)*innovation(:,j);
    if D2(j)<=chi2inv(0.999,1)
        pf(j).weight=pf(j).weight*1/sqrt(det(2*pi*Q))*exp(-0.5*D2(j));
        y(j)=1;
    else
        y(j)=0;
...
for j=1:Np
    if y(j)==1
        ...
        K=pf(j).cov{v_id}*C.'*(R+C*pf(j).cov{v_id}*C.')^-1;
        pf(j).mu{v_id}=pf(j).mu{v_id}+K*innovation(:,j);
        pf(j).cov{v_id}=(eye(4)-K*C)*pf(j).cov{v_id};   %for large multipath error, EKF estimation can be over-confident, try using schmidtz EKF instead
    end
end
%     end
end
