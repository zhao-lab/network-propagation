function weight_CMM_new(m,vehicle_id,ang,distance)
global pf;
N_mc=100;  % #of sample for Monte Carlo integration
Np=length(pf{m});
road_normal=[sin(ang);-cos(ang)];
for k=1:Np
    sample(:,1:2)=mvnrnd([pf{m}(k).mu{vehicle_id}(1), ...
        pf{m}(k).mu{vehicle_id}(3)],pf{m}(k).cov{vehicle_id}([1,3],[1,3]),N_mc);
...
%         pf(k).weight=0.017;
%     else
        pf{m}(k).weight=pf{m}(k).weight*sum((sample*road_normal<1));
%     end
