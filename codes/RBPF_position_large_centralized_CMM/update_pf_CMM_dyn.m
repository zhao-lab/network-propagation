function update_pf_CMM_dyn(pose_measure,svxyzmat,sigma_thermal2,orgxyz,Np,v_id,hor_DLP,H,Ns,i)  % The last argument is the vehicle id
global pf;
...
for k=1:Np
    pf(k).mu{v_id}(1) = pf(k).mu{v_id}(1) + 5*sin(2*pi*i/(Ns-1));
    pf(k).mu{v_id}(3) = pf(k).mu{v_id}(3) - 5*cos(2*pi*i/(Ns-1));
