function predict_pf_CMM_new(m,sigma_a2,sigma_b2,sigma_d2,deltat,N,Np,Nsv,hor_DLP)
...
%prediction
for j=1:Np
    pf{m}(j).common=pf{m}(j).common+deltat*randn(Nsv,1)*5;
    for i=1:N
        pf{m}(j).mu{i}=aug_A*pf{m}(j).mu{i};
        pf{m}(j).cov{i}=aug_A*pf{m}(j).cov{i}*aug_A.';
        if mod(i,4)==1||mod(i,4)==2
            pf{m}(j).cov{i}=pf{m}(j).cov{i}+Sigma;
        end
        if mod(i,4)==3||mod(i,4)==0
            pf{m}(j).cov{i}=pf{m}(j).cov{i}+Sigma_2;
        end
    end
end
