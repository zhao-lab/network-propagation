function update_pf_CMM(pose_measure,svxyzmat,sigma_thermal2,orgxyz,Np,v_id,hor_DLP,H,pf_id)  % The last argument is the vehicle id
global pf;
...
for k=1:length(pf{pf_id})
    pose_local=[pf{pf_id}(k).mu{v_id}(1);pf{pf_id}(k).mu{v_id}(3)];  
    %pose_xyz=enu2xyz(pose_local,orgxyz);  %give xyz as column vector
    a=size(H,1);  %calculate the acutal number of satellites
    pre_bias=inv(H.'*H)*H.'*pf{pf_id}(k).common(1:a);
...
%     for k=1:sv_n
%         if y(j,k)==1
            Q=pf{pf_id}(j).cov{v_id}([1,3],[1,3])+hor_DLP*sigma_thermal2;
            D2(j)=innovation(:,j)'*inv(Q)*innovation(:,j);
            if D2(j)<=chi2inv(0.999,1)
            pf{pf_id}(j).weight=pf{pf_id}(j).weight*1/sqrt(det(2*pi*Q))*exp(-0.5*D2(j));
            y(j)=1;
            else
            y(j)=0;    
            pf{pf_id}(j).weight=pf{pf_id}(j).weight*p0;
            end
%     end
    end
%     innovation(:,1)
%     y(1,:)
    ...
    K=pf{pf_id}(j).cov{v_id}*C.'*(R+C*pf{pf_id}(j).cov{v_id}*C.')^-1;
    pf{pf_id}(j).mu{v_id}=pf{pf_id}(j).mu{v_id}+K*innovation(:,j);
    pf{pf_id}(j).cov{v_id}=(eye(4)-K*C)*pf{pf_id}(j).cov{v_id};   %for large multipath error, EKF estimation can be over-confident, try using schmidtz EKF instead
    pf{pf_id}(j).cov{v_id}=0.5*(pf{pf_id}(j).cov{v_id}+pf{pf_id}(j).cov{v_id}.');   %make sure it is symmetric
    end
    end
%     end
end
